...
var _ = Describe("RoundTripper", func() {
	...
	Context("reusing udpconn", func() {
		var originalDialer func(ctx context.Context, pconn net.PacketConn, remoteAddr net.Addr, host string, tlsConf *tls.Config, config *quic.Config, multiPath uint8) (quic.EarlyConnection, error)
		var req1, req2 *http.Request
		...
		It("reuses udpconn in different hosts", func() {
			Expect(rt.udpConn).To(BeNil())
			quicDialer = func(_ context.Context, pconn net.PacketConn, _ net.Addr, _ string, _ *tls.Config, _ *quic.Config, _ uint8) (quic.EarlyConnection, error) {

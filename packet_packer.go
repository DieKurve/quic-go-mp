...
type packerMP interface {
	PackCoalescedPacket(onlyAck bool, v protocol.VersionNumber) (*coalescedPacketMP, error)
	PackPacket(onlyAck bool, now time.Time, v protocol.VersionNumber) (shortHeaderPacketMP, *packetBuffer, error)
	MaybePackProbePacket(protocol.EncryptionLevel, protocol.VersionNumber) (*coalescedPacketMP, error)
	PackConnectionClose(*qerr.TransportError, protocol.VersionNumber) (*coalescedPacketMP, error)
	PackApplicationClose(*qerr.ApplicationError, protocol.VersionNumber) (*coalescedPacketMP, error)

	PackPathAbandon(*qerr.ApplicationError, uint64, protocol.VersionNumber) (*coalescedPacketMP, error)
	SetMaxPacketSize(protocol.ByteCount)
	PackMTUProbePacket(ping ackhandler.Frame, size protocol.ByteCount, now time.Time, v protocol.VersionNumber) (shortHeaderPacketMP, *packetBuffer, error)
...
type longHeaderPacketMP struct {
	header *wire.ExtendedHeader
	ack    *wire.AckMPFrame
	frames []*ackhandler.Frame
...
type shortHeaderPacketMP struct {
	*ackhandler.Packet
	// used for logging
	DestConnID      protocol.ConnectionID
	Ack             *wire.AckMPFrame
...
func (p shortHeaderPacketMP) IsAckEliciting() bool {
...
type coalescedPacketMP struct {
	buffer         *packetBuffer
	longHdrPackets []*longHeaderPacketMP
	shortHdrPacket *shortHeaderPacketMP
}
...
func (p *longHeaderPacketMP) EncryptionLevel() protocol.EncryptionLevel {
	//nolint:exhaustive // Will never be called for Retry packets (and they don't have encrypted data).
	switch p.header.Type {
	case protocol.PacketTypeInitial:
		return protocol.EncryptionInitial
...
func (p *longHeaderPacketMP) IsAckEliciting() bool { return ackhandler.HasAckElicitingFrames(p.frames) }
...
func (p *longHeaderPacketMP) ToAckHandlerPacket(now time.Time, q *retransmissionQueue) *ackhandler.Packet {
	largestAcked := protocol.InvalidPacketNumber
	if p.ack != nil {
		largestAcked = p.ack.LargestAcked()
	}
...
type ackMPFrameSource interface {
...
func newPacketPackerMP(srcConnID protocol.ConnectionID, getDestConnID func() protocol.ConnectionID, initialStream cryptoStream, handshakeStream cryptoStream, packetNumberManager packetNumberManager, retransmissionQueue *retransmissionQueue, remoteAddr net.Addr, cryptoSetup sealingManager, framer frameSource, acks ackMPFrameSource, datagramQueue *datagramQueue, perspective protocol.Perspective) *packetPackerMP {
	return &packetPackerMP{
		cryptoSetup:         cryptoSetup,
		getDestConnID:       getDestConnID,
		srcConnID:           srcConnID,
		initialStream:       initialStream,
...
func (p *packetPackerMP) PackConnectionClose(e *qerr.TransportError, v protocol.VersionNumber) (*coalescedPacketMP, error) {
	var reason string
	// don't send details of crypto errors
	if !e.ErrorCode.IsCryptoError() {
		reason = e.ErrorMessage
...
// PackApplicationClose packs a packet that closes the connection with an application error.
func (p *packetPackerMP) PackApplicationClose(e *qerr.ApplicationError, v protocol.VersionNumber) (*coalescedPacketMP, error) {
	return p.packConnectionClose(true, uint64(e.ErrorCode), 0, e.ErrorMessage, v)
}
...
func (p *packetPackerMP) packConnectionClose(
	isApplicationError bool,
	errorCode uint64,
	frameType uint64,
	reason string,
	v protocol.VersionNumber,
) (*coalescedPacketMP, error) {
	var sealers [4]sealer
	var hdrs [3]*wire.ExtendedHeader
	var payloads [4]payloadMP
	var size protocol.ByteCount
	var connID protocol.ConnectionID
	var oneRTTPacketNumber protocol.PacketNumber
	var oneRTTPacketNumberLen protocol.PacketNumberLen
	...
	buffer := getPacketBuffer()
	packet := &coalescedPacketMP{
		buffer:         buffer,
		longHdrPackets: make([]*longHeaderPacketMP, 0, numLongHdrPackets),
	}
	for i, encLevel := range encLevels {
		if sealers[i] == nil {
			continue
		...
		if encLevel == protocol.Encryption1RTT {
			...
			packet.shortHdrPacket = &shortHeaderPacketMP{
				Packet:          ap,
				DestConnID:      connID,
				Ack:             ack,
				PacketNumberLen: oneRTTPacketNumberLen,
...
func (p *packetPackerMP) PackPathAbandon(e *qerr.ApplicationError, desConIDSeqNum uint64, v protocol.VersionNumber) (*coalescedPacketMP, error) {
	return p.packPathAbandon(true, uint64(e.ErrorCode), 0, e.ErrorMessage, desConIDSeqNum, v)
}

func (p *packetPackerMP) packPathAbandon(
	isApplicationError bool,
	errorCode uint64,
	frameType uint64,
	reason string,
	desConIDSeqNum uint64,
	v protocol.VersionNumber,
) (*coalescedPacketMP, error) {
	...
	for i, encLevel := range encLevels {
		...
		ccf := &wire.PathAbandonFrame{
			DestinationConnectionIDSequenceNumber: desConIDSeqNum,
			ErrorCode:                             errorCode,
			FrameType:                             frameType,
			ReasonPhrase:                          reason,
		}
		// don't send application errors in Initial or Handshake packets
		if isApplicationError && (encLevel == protocol.EncryptionInitial || encLevel == protocol.EncryptionHandshake) {
			ccf.DestinationConnectionIDSequenceNumber = 0
			ccf.ErrorCode = uint64(qerr.ApplicationErrorErrorCode)
			ccf.ReasonPhrase = ""
		}
		pl := payloadMP{
...
// It takes into account that packets that have a tiny payload need to be padded,
// such that len(payload) + packet number len >= 4 + AEAD overhead
func (p *packetPackerMP) longHeaderPacketLength(hdr *wire.ExtendedHeader, pl payloadMP, v protocol.VersionNumber) protocol.ByteCount {
	var paddingLen protocol.ByteCount
	pnLen := protocol.ByteCount(hdr.PacketNumberLen)
	if pl.length < 4-pnLen {
		paddingLen = 4 - pnLen - pl.length
...
// It takes into account that packets that have a tiny payload need to be padded,
// such that len(payload) + packet number len >= 4 + AEAD overhead
func (p *packetPackerMP) shortHeaderPacketLength(connID protocol.ConnectionID, pnLen protocol.PacketNumberLen, pl payloadMP) protocol.ByteCount {
	var paddingLen protocol.ByteCount
	if pl.length < 4-protocol.ByteCount(pnLen) {
		paddingLen = 4 - protocol.ByteCount(pnLen) - pl.length
	}
...
// size is the expected size of the packet, if no padding was applied.
func (p *packetPackerMP) initialPaddingLen(frames []*ackhandler.Frame, size protocol.ByteCount) protocol.ByteCount {
	// For the server, only ack-eliciting Initial packets need to be padded.
	if p.perspective == protocol.PerspectiveServer && !ackhandler.HasAckElicitingFrames(frames) {
		return 0
	}
...
// It packs an Initial / Handshake if there is data to send in these packet number spaces.
// It should only be called before the handshake is confirmed.
func (p *packetPackerMP) PackCoalescedPacket(onlyAck bool, v protocol.VersionNumber) (*coalescedPacketMP, error) {
	maxPacketSize := p.maxPacketSize
	if p.perspective == protocol.PerspectiveClient {
		maxPacketSize = protocol.MinInitialPacketSize
	}
	var (
		initialHdr, handshakeHdr, zeroRTTHdr                            *wire.ExtendedHeader
		initialPayload, handshakePayload, zeroRTTPayload, oneRTTPayload payloadMP
		oneRTTPacketNumber                                              protocol.PacketNumber
		oneRTTPacketNumberLen                                           protocol.PacketNumberLen
	)
	// Try packing an Initial packet.
	...
	packet := &coalescedPacketMP{
		buffer:         buffer,
		longHdrPackets: make([]*longHeaderPacketMP, 0, 3),
	}
	if initialPayload.length > 0 {
		padding := p.initialPaddingLen(initialPayload.frames, size)
		cont, err := p.appendLongHeaderPacket(buffer, initialHdr, initialPayload, padding, protocol.EncryptionInitial, initialSealer, v)
...
func (p *packetPackerMP) PackPacket(onlyAck bool, now time.Time, v protocol.VersionNumber) (shortHeaderPacketMP, *packetBuffer, error) {
	sealer, err := p.cryptoSetup.Get1RTTSealer()
	if err != nil {
		return shortHeaderPacketMP{}, nil, err
	}
	pn, pnLen := p.pnManager.PeekPacketNumber(protocol.Encryption1RTT)
	connID := p.getDestConnID()
	hdrLen := wire.ShortHeaderLen(connID, pnLen)
	pl := p.maybeGetShortHeaderPacket(sealer, hdrLen, p.maxPacketSize, onlyAck, true, v)
	if pl.length == 0 {
		return shortHeaderPacketMP{}, nil, errNothingToPack
	}
	kp := sealer.KeyPhase()
	buffer := getPacketBuffer()
	ap, ack, err := p.appendShortHeaderPacket(buffer, connID, pn, pnLen, kp, pl, 0, sealer, false, v)
...
func (p *packetPackerMP) maybeGetCryptoPacket(maxPacketSize protocol.ByteCount, encLevel protocol.EncryptionLevel, onlyAck, ackAllowed bool, v protocol.VersionNumber) (*wire.ExtendedHeader, payloadMP) {
	if onlyAck {
		if ack := p.acks.GetAckMPFrame(encLevel, true); ack != nil {
			return p.getLongHeader(encLevel, v), payloadMP{
				ack:    ack,
				length: ack.Length(v),
			}
		}
		return nil, payloadMP{}
	}
	...
	hasData := s.HasData()
	var ack *wire.AckMPFrame
	if ackAllowed {
		ack = p.acks.GetAckMPFrame(encLevel, !hasRetransmission && !hasData)
	}
	if !hasData && !hasRetransmission && ack == nil {
		// nothing to send
		return nil, payloadMP{}
...
func (p *packetPackerMP) maybeGetAppDataPacketFor0RTT(sealer sealer, maxPacketSize protocol.ByteCount, v protocol.VersionNumber) (*wire.ExtendedHeader, payloadMP) {
	if p.perspective != protocol.PerspectiveClient {
		return nil, payloadMP{}
	}
...
func (p *packetPackerMP) maybeGetShortHeaderPacket(sealer handshake.ShortHeaderSealer, hdrLen protocol.ByteCount, maxPacketSize protocol.ByteCount, onlyAck, ackAllowed bool, v protocol.VersionNumber) payloadMP {
	maxPayloadSize := maxPacketSize - hdrLen - protocol.ByteCount(sealer.Overhead())
	return p.maybeGetAppDataPacket(maxPayloadSize, onlyAck, ackAllowed, v)
}
...
func (p *packetPackerMP) maybeGetAppDataPacket(maxPayloadSize protocol.ByteCount, onlyAck, ackAllowed bool, v protocol.VersionNumber) payloadMP {
	pl := p.composeNextPacket(maxPayloadSize, onlyAck, ackAllowed, v)
...
func (p *packetPackerMP) composeNextPacket(maxFrameSize protocol.ByteCount, onlyAck, ackAllowed bool, v protocol.VersionNumber) payloadMP {
	if onlyAck {
		if ack := p.acks.GetAckMPFrame(protocol.Encryption1RTT, true); ack != nil {
			return payloadMP{
				ack:    ack,
				length: ack.Length(v),
			}
	...
	pl := payloadMP{frames: make([]*ackhandler.Frame, 0, 1)}
	...
	var hasAck bool
	if ackAllowed {
		if ack := p.acks.GetAckMPFrame(protocol.Encryption1RTT, !hasRetransmission && !hasData); ack != nil {
			pl.ack = ack
			pl.length += ack.Length(v)
			hasAck = true
		}
...
func (p *packetPackerMP) MaybePackProbePacket(encLevel protocol.EncryptionLevel, v protocol.VersionNumber) (*coalescedPacketMP, error) {
	if encLevel == protocol.Encryption1RTT {
		s, err := p.cryptoSetup.Get1RTTSealer()
		if err != nil {
			return nil, err
		...
		buffer := getPacketBuffer()
		packet := &coalescedPacketMP{buffer: buffer}
		ap, ack, err := p.appendShortHeaderPacket(buffer, connID, pn, pnLen, kp, pl, 0, s, false, v)
		if err != nil {
			return nil, err
		}
	...
	packet.longHdrPackets = []*longHeaderPacketMP{longHdrPacket}
	return packet, nil
}
...
func (p *packetPackerMP) PackMTUProbePacket(ping ackhandler.Frame, size protocol.ByteCount, now time.Time, v protocol.VersionNumber) (shortHeaderPacketMP, *packetBuffer, error) {
	pl := payloadMP{
		frames: []*ackhandler.Frame{&ping},
		length: ping.Length(v),
	}
...
func (p *packetPackerMP) getLongHeader(encLevel protocol.EncryptionLevel, v protocol.VersionNumber) *wire.ExtendedHeader {
	pn, pnLen := p.pnManager.PeekPacketNumber(encLevel)
	hdr := &wire.ExtendedHeader{
		PacketNumber:    pn,
		PacketNumberLen: pnLen,
...
func (p *packetPackerMP) appendLongHeaderPacket(buffer *packetBuffer, header *wire.ExtendedHeader, pl payloadMP, padding protocol.ByteCount, encLevel protocol.EncryptionLevel, sealer sealer, v protocol.VersionNumber) (*longHeaderPacketMP, error) {
	var paddingLen protocol.ByteCount
	pnLen := protocol.ByteCount(header.PacketNumberLen)
	if pl.length < 4-pnLen {
		paddingLen = 4 - pnLen - pl.length
...
func (p *packetPackerMP) appendShortHeaderPacket(
	buffer *packetBuffer,
	connID protocol.ConnectionID,
	pn protocol.PacketNumber,
	pnLen protocol.PacketNumberLen,
	...
	isMTUProbePacket bool,
	v protocol.VersionNumber,
) (*ackhandler.Packet, *wire.AckMPFrame, error) {
	var paddingLen protocol.ByteCount
	if pl.length < 4-protocol.ByteCount(pnLen) {
		paddingLen = 4 - protocol.ByteCount(pnLen) - pl.length
	}
...
func (p *packetPackerMP) appendPacketPayload(raw []byte, pl payloadMP, paddingLen protocol.ByteCount, v protocol.VersionNumber) ([]byte, error) {
	payloadOffset := len(raw)
	if pl.ack != nil {
		var err error
		raw, err = pl.ack.Append(raw, v)
...
func (p *packetPackerMP) encryptPacket(raw []byte, sealer sealer, pn protocol.PacketNumber, payloadOffset, pnLen protocol.ByteCount) []byte {
	_ = sealer.Seal(raw[payloadOffset:payloadOffset], raw[payloadOffset:], pn, raw[:payloadOffset])
	raw = raw[:len(raw)+sealer.Overhead()]
	// apply header protection
	pnOffset := payloadOffset - pnLen
...
func (p *packetPackerMP) SetToken(token []byte) {
	p.token = token
}
...
// When a higher MTU is discovered, use it.
func (p *packetPackerMP) SetMaxPacketSize(s protocol.ByteCount) {
	p.maxPacketSize = s
}
...
// If the peer sets a max_packet_size that's smaller than the size we're currently using,
// we need to reduce the size of packets we send.
func (p *packetPackerMP) HandleTransportParameters(params *wire.TransportParameters) {
	if params.MaxUDPPayloadSize != 0 {
		p.maxPacketSize = utils.Min(p.maxPacketSize, params.MaxUDPPayloadSize)
	}
}

...
type receivedPacketHandler struct {
	...
	initialPackets   *singlePathTracker
	handshakePackets *singlePathTracker
	appDataPackets   *singlePathTracker

	lowest1RTTPacket protocol.PacketNumber
}

type receivedMultiPathPacketHandler struct {
	sentPackets sentPacketTracker

	initialPackets   *multiPathTracker
	handshakePackets *multiPathTracker
	appDataPackets   *multiPathTracker
...
var _ ReceivedMPPacketHandler = &receivedMultiPathPacketHandler{}

func newReceivedSinglePathPacketHandler(
	sentPackets sentPacketTracker,
...
func newReceivedMultiPathPacketHandler(
	sentPackets sentPacketTracker,
	rttStats *utils.RTTStats,
	logger utils.Logger,
) ReceivedMPPacketHandler {
	return receivedMultiPathPacketHandler{
		sentPackets:      sentPackets,
		initialPackets:   newReceivedMultiPathPacketTracker(rttStats, logger),
		handshakePackets: newReceivedMultiPathPacketTracker(rttStats, logger),
		appDataPackets:   newReceivedMultiPathPacketTracker(rttStats, logger),
		lowest1RTTPacket: protocol.InvalidPacketNumber,
	}
}
...
func (h receivedMultiPathPacketHandler) ReceivedPacket(
	pn protocol.PacketNumber,
	ecn protocol.ECN,
	encLevel protocol.EncryptionLevel,
	rcvTime time.Time,
	...
	switch encLevel {
	case protocol.EncryptionInitial:
		return h.initialPackets.ReceivedMPPacket(pn, ecn, rcvTime, shouldInstigateAck)
	case protocol.EncryptionHandshake:
		return h.handshakePackets.ReceivedMPPacket(pn, ecn, rcvTime, shouldInstigateAck)
	case protocol.Encryption0RTT:
		if h.lowest1RTTPacket != protocol.InvalidPacketNumber && pn > h.lowest1RTTPacket {
			return fmt.Errorf("received packet number %d on a 0-RTT packet after receiving %d on a 1-RTT packet", pn, h.lowest1RTTPacket)
		}
		return h.appDataPackets.ReceivedMPPacket(pn, ecn, rcvTime, shouldInstigateAck)
	case protocol.Encryption1RTT:
		if h.lowest1RTTPacket == protocol.InvalidPacketNumber || pn < h.lowest1RTTPacket {
			h.lowest1RTTPacket = pn
		}
		if err := h.appDataPackets.ReceivedMPPacket(pn, ecn, rcvTime, shouldInstigateAck); err != nil {
			return err
		}
		h.appDataPackets.IgnoreBelow(h.sentPackets.GetLowestPacketNotConfirmedAcked())
		return nil
...
func (h receivedMultiPathPacketHandler) DropPackets(encLevel protocol.EncryptionLevel) {
	switch encLevel {
	case protocol.EncryptionInitial:
		h.initialPackets = nil
	case protocol.EncryptionHandshake:
...
func (h receivedMultiPathPacketHandler) GetAlarmTimeout() time.Time {
	var initialAlarm, handshakeAlarm time.Time
	if h.initialPackets != nil {
		initialAlarm = h.initialPackets.GetAlarmTimeout()
	}
...
func (h receivedMultiPathPacketHandler) GetAckMPFrame(encLevel protocol.EncryptionLevel, onlyIfQueued bool) *wire.AckMPFrame {
	var ack *wire.AckMPFrame
	//nolint:exhaustive // 0-RTT packets can't contain ACK frames.
	switch encLevel {
	case protocol.EncryptionInitial:
		if h.initialPackets != nil {
			ack = h.initialPackets.GetAckMPFrame(onlyIfQueued)
		}
	case protocol.EncryptionHandshake:
		if h.handshakePackets != nil {
			ack = h.handshakePackets.GetAckMPFrame(onlyIfQueued)
		}
	case protocol.Encryption1RTT:
		// 0-RTT packets can't contain ACK frames
		return h.appDataPackets.GetAckMPFrame(onlyIfQueued)
	default:
		return nil
	}
	// For Initial and Handshake ACKs, the delay time is ignored by the receiver.
...
func (h receivedMultiPathPacketHandler) IsPotentiallyDuplicate(pn protocol.PacketNumber, encLevel protocol.EncryptionLevel) bool {
	switch encLevel {
	case protocol.EncryptionInitial:
		if h.initialPackets != nil {
			return h.initialPackets.IsPotentiallyDuplicate(pn)

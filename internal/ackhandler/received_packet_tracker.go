...
type singlePathTracker struct {
	*receivedPacketTracker
	lastAck *wire.AckFrame
}

type multiPathTracker struct {
	*receivedPacketTracker
	lastAck *wire.AckMPFrame
}
...
func newReceivedMultiPathPacketTracker(
	rttStats *utils.RTTStats,
	logger utils.Logger,
) *multiPathTracker {
	return &multiPathTracker{
		receivedPacketTracker: &receivedPacketTracker{
			packetHistory: newReceivedPacketHistory(),
			maxAckDelay:   protocol.MaxAckDelay,
			rttStats:      rttStats,
...
func (h *singlePathTracker) ReceivedPacket(packetNumber protocol.PacketNumber, ecn protocol.ECN, rcvTime time.Time, shouldInstigateAck bool) error {
...
func (h *multiPathTracker) ReceivedMPPacket(packetNumber protocol.PacketNumber, ecn protocol.ECN, rcvTime time.Time, shouldInstigateAck bool) error {
	if isNew := h.packetHistory.ReceivedPacket(packetNumber); !isNew {
		return fmt.Errorf("recevedPacketTracker BUG: ReceivedPacket called for old / duplicate packet %d", packetNumber)
	}

	isMissing := h.isMissingMP(packetNumber)
	if packetNumber >= h.largestObserved {
		h.largestObserved = packetNumber
		h.largestObservedReceivedTime = rcvTime
	}
	...
	if shouldInstigateAck {
		h.maybeQueueAckMP(packetNumber, rcvTime, isMissing)
	}
	switch ecn {
	case protocol.ECNNon:
	case protocol.ECT0:
...
func (h *singlePathTracker) isMissing(p protocol.PacketNumber) bool {
...
func (h *multiPathTracker) isMissingMP(p protocol.PacketNumber) bool {
	if h.lastAck == nil || p < h.ignoreBelow {
		return false
	}
	return p < h.lastAck.LargestAcked() && !h.lastAck.AcksPacket(p)
...
func (h *singlePathTracker) hasNewMissingPackets() bool {
	if h.lastAck == nil {
		return false
	}
...
func (h *multiPathTracker) hasNewMissingPacketsMP() bool {
	if h.lastAck == nil {
		return false
	}
	highestRange := h.packetHistory.GetHighestAckRange()
...
func (h *singlePathTracker) maybeQueueAck(pn protocol.PacketNumber, rcvTime time.Time, wasMissing bool) {
...
func (h *multiPathTracker) maybeQueueAckMP(pn protocol.PacketNumber, rcvTime time.Time, wasMissing bool) {
	// always acknowledge the first packet
	if h.lastAck == nil {
		if !h.ackQueued {
...
func (h *singlePathTracker) GetAckFrame(onlyIfQueued bool) *wire.AckFrame {
	if !h.hasNewAck {
		return nil
	}
	now := time.Now()
...
func (h *multiPathTracker) GetAckMPFrame(onlyIfQueued bool) *wire.AckMPFrame {
	if !h.hasNewAck {
		return nil
	}
	now := time.Now()

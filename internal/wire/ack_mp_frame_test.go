...
var _ = Describe("ACK MP Frame (for IETF QUIC)", func() {
	Context("parsing", func() {
		...
		It("uses the ack delay exponent", func() {
			...
			for i := uint8(0); i < 8; i++ {
				...
				Expect(err).ToNot(HaveOccurred())
				frame, err := parseAckMPFrame(r, typ, protocol.AckDelayExponent+i, protocol.Version1)
				Expect(err).ToNot(HaveOccurred())
		...
		It("errors on EOF", func() {
			...
			for i := range data {
				_, err := parseAckMPFrame(bytes.NewReader(data[:i]), ackMPFrameType, protocol.AckDelayExponent, protocol.Version1)
		...
		Context("ACK_ECN", func() {
			It("parses", func() {
				...
				b := bytes.NewReader(data)
				frame, err := parseAckMPFrame(b, ackMPECNFrameType, protocol.AckDelayExponent, protocol.Version1)
				Expect(err).ToNot(HaveOccurred())
				Expect(frame.LargestAcked()).To(Equal(protocol.PacketNumber(100)))
				Expect(frame.LowestAcked()).To(Equal(protocol.PacketNumber(90)))
				Expect(frame.HasMissingRanges()).To(BeFalse())
	...
	Context("when writing", func() {
		...
		It("writes a frame that acks a single packet", func() {
			...
			typ, err := quicvarint.Read(r)
			Expect(err).ToNot(HaveOccurred())
			frame, err := parseAckMPFrame(r, typ, protocol.AckDelayExponent, protocol.Version1)
			Expect(err).ToNot(HaveOccurred())
		...
		It("limits the maximum size of the ACK frame", func() {
			...
			f := &AckMPFrame{AckRanges: ackRanges}
			Expect(f.validateAckRanges()).To(BeTrue())
			b, err := f.Append(nil, protocol.Version1)
			Expect(err).ToNot(HaveOccurred())
			Expect(b).To(HaveLen(int(f.Length(protocol.Version1))))
	...
	Context("ACK range validator", func() {
		It("rejects ACKs without ranges", func() {
			Expect((&AckMPFrame{}).validateAckRanges()).To(BeFalse())
		})

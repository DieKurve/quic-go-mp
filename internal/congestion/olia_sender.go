...
func NewOliaSender(oliaSenders map[protocol.PathID]*OliaSender, rttStats *utils.RTTStats, initialCongestionWindow, initialMaxCongestionWindow protocol.PacketNumber) SendAlgorithmWithDebugInfo {
	return &OliaSender{
		rttStats:                   rttStats,
...
func (o *OliaSender) TimeUntilSend(now time.Time, bytesInFlight protocol.ByteCount) time.Time {
	if o.InRecovery() {
...
func (o *OliaSender) GetCongestionWindow() protocol.ByteCount {
	return protocol.ByteCount(o.congestionWindow) * protocol.MaxDatagramFrameSize
}

func (o *OliaSender) GetSlowStartThreshold() protocol.ByteCount {
	return protocol.ByteCount(o.slowstartThreshold) * maxDatagramSize
}
...
func (o *OliaSender) MaybeExitSlowStart() {
	if o.InSlowStart() && o.hybridSlowStart.ShouldExitSlowStart(o.rttStats.LatestRTT(), o.rttStats.MinRTT(), o.GetCongestionWindow()/maxDatagramSize) {
		o.ExitSlowstart()
	}
}
...
func (o *OliaSender) OnPacketAcked(ackedPacketNumber protocol.PacketNumber, ackedBytes protocol.ByteCount, bytesInFlight protocol.ByteCount) {
	o.largestAckedPacketNumber = utils.Max(ackedPacketNumber, o.largestAckedPacketNumber)
	if o.InRecovery() {
		// PRR is used when in recovery
	...
	if packetNumber <= o.largestSentAtLastCutback {
		if o.lastCutbackExitedSlowstart {
			...
			if o.slowStartLargeReduction {
				if o.stats.slowstartPacketsLost == 1 || (o.stats.slowstartBytesLost/maxDatagramSize) > (o.stats.slowstartBytesLost-lostBytes)/maxDatagramSize {
					// Reduce congestion window by 1 for every mss of bytes lost.
					o.congestionWindow = utils.Max(o.congestionWindow-1, o.minCongestionWindow)
				}
				o.slowstartThreshold = o.congestionWindow
			}
		}
